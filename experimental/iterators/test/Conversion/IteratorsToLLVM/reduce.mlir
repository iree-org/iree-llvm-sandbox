...
// CHECK-LABEL: func private @iterators.reduce.Close.{{[0-9]+}}(%{{.*}}: !llvm.struct<"iterators.reduceState{{.*}}", ({{.*}})>) -> !llvm.struct<"iterators.reduceState{{.*}}", ({{.*}})> {
// CHECK-NEXT:    %[[V0:.*]] = llvm.extractvalue %[[arg0:.*]][0 : index] : !llvm.struct<"[[reduceStateName:iterators\.reduceState.*]]", ([[nestedUpstreamStateType:.*]])>
...
// CHECK-LABEL: func private @iterators.reduce.Next.{{[0-9]+}}(%{{.*}}: !llvm.struct<"iterators.reduceState{{.*}}", ({{.*}})>) -> (!llvm.struct<"iterators.reduceState{{.*}}", ({{.*}})>, i1, !llvm.struct<(i32)>)
// CHECK-NEXT:    %[[V0:.*]] = llvm.extractvalue %[[arg0:.*]][0 : index] : !llvm.struct<"[[reduceStateName:iterators\.reduceState.*]]", ([[nestedUpstreamStateType:.*]])>
...
// CHECK-LABEL: func private @iterators.reduce.Open.{{[0-9]+}}(%{{.*}}: !llvm.struct<"iterators.reduceState{{.*}}", ({{.*}})>) -> !llvm.struct<"iterators.reduceState{{.*}}", ({{.*}})>
// CHECK-NEXT:    %[[V0:.*]] = llvm.extractvalue %[[arg0:.*]][0 : index] : !llvm.struct<"[[reduceStateName:iterators\.reduceState.*]]", ([[nestedUpstreamStateType:.*]])>
...
func @main() {
  ...
  // CHECK:        %[[V1:.*]] = llvm.mlir.undef : !llvm.struct<"[[reduceStateName:iterators\.reduceState.*]]", ([[nestedUpstreamStateType:.*]])>
