...
func.func @test_non_empty() {
  ...
  %stream = iterators.value_to_stream %value : <!llvm.struct<(i32)>>
  %result:2 = iterators.stream_to_value %stream : <!llvm.struct<(i32)>>
  scf.if %result#1 {
...
func.func @test_empty() {
  ...
  %filtered = "iterators.filter"(%stream) {predicateRef = @return_false}
    : (!iterators.stream<!llvm.struct<(i32)>>)
      -> (!iterators.stream<!llvm.struct<(i32)>>)
  %result:2 = iterators.stream_to_value %filtered : <!llvm.struct<(i32)>>
  ...
  // executed.
  %tuple = "iterators.constanttuple"() { values = [12345 : i32] } : () -> tuple<i32>
