...
/// Lowers create_tabular_view to LLVM IR that extracts the bare pointers and
...
struct CreateTabularViewOpLowering
    : public OpConversionPattern<CreateTabularViewOp> {
  CreateTabularViewOpLowering(TypeConverter &typeConverter,
                              MLIRContext *context, PatternBenefit benefit = 1)
      : OpConversionPattern(typeConverter, context, benefit) {}

  LogicalResult
  matchAndRewrite(CreateTabularViewOp op, OpAdaptor adaptor,
...
void mlir::iterators::populateTabularToLLVMConversionPatterns(
  ...
  patterns.add<CreateTabularViewOpLowering>(typeConverter,
                                            patterns.getContext());
}
