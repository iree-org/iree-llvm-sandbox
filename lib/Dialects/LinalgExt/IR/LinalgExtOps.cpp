...
static LogicalResult verify(InParallelOp op) {
  ...
  auto terminatorTypes =
      llvm::cast<PerformConcurrentlyOp>(body->getTerminator()).resultTypes();
  auto opResults = op.getResults();
  if (opResults.size() != terminatorTypes.size())
    return op.emitOpError("has ")
           << opResults.size() << " results, but its terminator produces "
  ...
  for (auto e : llvm::zip(terminatorTypes, opResults)) {
    if (std::get<0>(e) != std::get<1>(e).getType())
      return op.emitOpError()
             ...
             << std::get<0>(e) << ") and " << i << "th result produced by its "
...
void InParallelOp::build(
  ...
  if (!bodyBuilder) {
    ...
    result.addTypes(terminator.resultTypes());
  }
}
