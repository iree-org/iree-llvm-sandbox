...
def version_key():
  ...
  # ptxas version
  try:
    ptxas_version = hashlib.md5(subprocess.check_output(["ptxas", "--version"
                                                        ])).hexdigest()
  except Exception:
    ptxas_version = ''
  return '-'.join(
      triton.__version__) + '-' + ptxas_version + '-' + '-'.join(contents)
...
class JITFunction(KernelInterface[T]):
  ...
  def _get_config(self, *args):
    ...
    return namedtuple("instance_descriptor",
                      ["divisible_by_16", "equal_to_1"])(tuple(divisible_by_16),
                                                         tuple(equal_to_1))
  ...
  def _call_hook(self, key, signature, device, constants, num_warps, num_stages,
                 extern_libs, configs):
    ...
    return JITFunction.cache_hook(key=key,
                                  repr=repr,
  ...
  def _make_launcher(self):
    regular_args = [
        f'{arg}' for i, arg in enumerate(self.arg_names)
...
"""
  ...
  def __init__(self,
    ...
    self.kernel = None
    self.debug = os.environ.get("TRITON_DEBUG",
                                "0") == "1" if debug is None else debug
    self.noinline = noinline
    # annotations
    normalize_ty = lambda ty: ty.__name__ if isinstance(ty, type) else ty
    self.__annotations__ = {
        name: normalize_ty(ty) for name, ty in fn.__annotations__.items()
    }
    # index of constexprs
    self.constexprs = [
  ...
  def cache_key(self):
    ...
    if self.hash is None:
      dependencies_finder = DependenciesFinder(globals=self.__globals__,
                                               src=self.src)
