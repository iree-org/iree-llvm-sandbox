...
#               A     y    x
#   Layout: {{m, n}, {n}, {m}}
class MatVecProblem(ProblemDefinition):
  """ Problem definition for a single fill + matvec problem."""
  ...
  def gflop_count_builder(self, M: int, N: int) -> float:
    ...
    """
    return float(2.0 * M * N) / float(1e9)

  def gbyte_count_builder(self, M: int, N: int, lhs_np_type: np.dtype,
    ...
    """
    ro_gbytes = 1e-9 * (
        M * N * np.dtype(rhs_np_type).itemsize +
        N * np.dtype(acc_np_type).itemsize)
    rw_gbytes = 2e-9 * (M * np.dtype(rhs_np_type).itemsize)
    return ro_gbytes + rw_gbytes

  def tensors_np_builder(self, M: int, N: int, lhs_np_type: np.dtype,
                         rhs_np_type: np.dtype,
  ...
  def check_np(self, A: np.dtype, y: np.dtype, x: np.dtype) -> None:
  ...
  def build_problem_under_context_manager(self, name: str, lhs_mlir_type: Type,
    ...
    with InsertionPoint(func.add_entry_block()):
      ...
      # linalg.matvec returns a Value instead of OpView, so we have to manually
